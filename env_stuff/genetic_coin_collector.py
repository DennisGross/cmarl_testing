...
from env_stuff.coin_collector import *
import numpy as np
import math
...
class Individual:
    ...
    def mutate(self, mutation_rate):
        ...
        if random.random() < mutation_rate:
            self.x3 += random.randint(-self.MAX_X+1, self.MAX_X-1)
        if random.random() < mutation_rate:
            self.y3 += random.randint(-self.MAX_Y+1, self.MAX_Y-1)
        ...
        self.x3 = max(min(self.x3, self.MAX_X-1), 0)
        self.y3 = max(min(self.y3, self.MAX_Y-1), 0)
    ...
    def random_individual():
        indi = Individual(random.randint(-6, 6), random.randint(-7, 7), random.randint(-6, 6), random.randint(-7, 7), random.randint(-6, 6), random.randint(-7, 7))
        ...
        x3 = random.randint(0, indi.MAX_X-1)
        y3 = random.randint(0, indi.MAX_Y-1)
        return Individual(x1, y1, x2, y2, x3, y3)
...
class GGTest(GTest):
    ...
    def post_reset_configuration(self, next_state):
        ...
        # Update State
        self.env.unwrapped.x0 = next_individual.x1
        self.env.unwrapped.y0 = next_individual.y1
        self.env.unwrapped.x1 = next_individual.x2
        self.env.unwrapped.y1 = next_individual.y2
        self.env.unwrapped.x2 = next_individual.x3
        self.env.unwrapped.y2 = next_individual.y3
